...
export default function JourneyMode({ username, onBack }) {
  ...
  if (!getToken() || authExpired) {
    return (
      <div className="screen jm-screen">
        <div className="jm-gate-card">
          <span className="jm-gate-icon">🚑</span>
  ...
  if (error) {
    return (
      <div className="screen jm-screen">
        <div className="jm-gate-card">
          <span className="jm-gate-icon">🗺️</span>
  ...
  if (view === 'subjects') {
    return (
      <div className="screen jm-screen">
        <button className="ms-back-btn jm-back-btn" onClick={onBack}>← Back</button>
        <div className="jm-banner">
          <h1 className="jm-title">🚑 FIRST AID JOURNEY</h1>
          <p className="jm-tagline">Pick a subject and march through it, chapter by chapter</p>
        </div>
  ...
  if (chapters.length === 0) {
    return (
      <div className="screen jm-screen">
        <div className="jm-gate-card">
          <span className="jm-gate-icon">{subject?.icon || '🗺️'}</span>
          <h2>{subject?.label}</h2>
  ...
  // Alternating left/right rhythm around the center spine, continuous across chapters
  let slot = 0;
  const nextSide = () => (slot++ % 2 === 0 ? 'left' : 'right');
  ...
  const renderLevelNode = (l) => {
    ...
    const cls = [
      'jm-node', 'jm-node--level', `jm-node-row--${side}`,
    ...
    return (
      <div key={l.level_key} className={`jm-node-row jm-node-row--${side}`} ref={isFrontier ? frontierRef : null}>
        <button
          className={cls}
          disabled={!tappable}
          onClick={() => openConfirm({
        ...
        <div className="jm-node-caption">
          ...
          {empty && <span className="jm-node-tag">No questions yet</span>}
        </div>
      </div>
    );
  };
  ...
  const renderBossNode = (boss, label, big) => {
    const side       = big ? 'center' : nextSide();
    ...
    const cls = [
      'jm-node', big ? 'jm-node--ultimate' : 'jm-node--boss', `jm-node-row--${side}`,
    ...
    return (
      <div key={boss.level_key} className={`jm-node-row jm-node-row--${side}`} ref={isFrontier ? frontierRef : null}>
        <button
          className={cls}
          disabled={!tappable}
          onClick={() => openConfirm({
        ...
        <div className="jm-node-caption">
          ...
          {boss.auto_skipped && <span className="jm-node-tag">SKIPPED</span>}
  ...
  return (
    <div className="screen jm-screen jm-screen--path">
      <div className="jm-path-header">
        <button className="ms-back-btn jm-back-btn" onClick={() => { setView('subjects'); setPath(null); setConfirmNode(null); }}>
          ← Subjects
        </button>
